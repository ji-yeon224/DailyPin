...
final class MapKitManager: NSObject {
    ...
    func checkDeviceLocationAuthorization() {
        ...
        DispatchQueue.global().async { [weak self] in
            ...
            if CLLocationManager.locationServicesEnabled() {
                ...
                self.delegate?.failGetUserLoaction(message: LocalizableKey.locationServicesEnabled.localized)
            }
        }
    }
    ...
    private func checkCurrentLocationAuthorization(status: CLAuthorizationStatus) {
        ...
        case .restricted: // 위치 서비스 사용 권한이 없음
            delegate?.failGetUserLoaction(message: LocalizableKey.location_Restricted.localized)
...
extension MapKitManager: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        debugPrint("fail")
        delegate?.failGetUserLoaction(message: LocalizableKey.locationFail.localized)
    }
